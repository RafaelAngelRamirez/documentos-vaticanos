...
* **Narrador · speech-prep para voces integradas:** `prepareSpeechText` / `nextSpeakableIndex` normalizan el texto hablado (strip HTML/refs, colapsa basura mecánica) y **saltan** placeholders OCR (`[OCR: …omitido]`), vacíos y TOC basura para que la narración continua no vocalice basura. Cableado en `lector.component.ts` `speakFrom`. Tests: `npm run test:speech-prep`. Sin reescribir packs ni unir `que.dista`/`es.decir`.
* **OCR residual cleanup a+b+c (`ocr-abc-v1`):** (a) collapse spaced letters (`H I S T O R I A`→`HISTORIA`, `t i e r r a`→`tierra`); (b) blank TOC/dotted-leader garbage units via placeholder (array slots kept → stable `unitIndex`); (c) re-OCR queue `documentos/corpus/ocr-reocr-queue.json` for residual body noise. Code: `scripts-descarga/src/pipeline/repair_ocr_noise.ts`; CLI `npm run ocr:abc-apply` / `test:ocr-abc`; revisions `documentos/corpus/revisions/ocr-abc/`.

### Features

* **Lector · jerarquía visual de títulos:** niveles 1–3 (PARTE/LIBRO > SECCIÓN/CAPÍTULO/TÍTULO > ARTÍCULO) vía `headingLevel` en `speech-prep.logic.ts`; `app-punto` aplica `punto-heading--1|2|3` con tamaño/aire en cascada (tokens). Tests: `npm run test:speech-prep`.
* **Lector · títulos/secciones con mejor estética y narración calmada:** detección de unidades estructurales (PARTE / SECCIÓN / CAPÍTULO / ARTÍCULO / PRÓLOGO / ALL-CAPS cortos sin `consecutivo` de artículo) en `speech-prep.logic.ts`. Visual en `app-punto` (clases `punto-heading` + `tsec`, centrado, serif, más aire; sin numeral de párrafo). Voz: title-case suave, pausa tras la etiqueta (`Capítulo Primero. …`), ritmo más lento (`rateScale` 0.82) en `speakFrom`. Tests: `npm run test:speech-prep`. Sin reescribir packs ni unir `que.dista`.
* **Narrador · voces Grok (xAI TTS) con API key por dispositivo:** en Ajustes cada equipo guarda su propia `xaiApiKey` de console.x.ai en `localStorage` (`dv.narr.prefs.v1`; **no** SuperGrok, **no** nube ni backup de la llave). El cliente llama a `api.x.ai/v1/tts` con esa clave; sin clave/red → solo Web Speech / Capacitor. Toggle Grok + voz preferida por dispositivo. Tests: `yarn test:narrator-grok`.
