/**
 * Runner for santoral parse tests via ts-node (loads .ts parsers with relative imports).
 * Assertions live in santoral_parse.test.js logic, inlined here so shipped parsers are exercised.
 */
import fs from 'fs';
...
const parsed = parseSaintBioHtml(html, SOURCE_URL);
...
const slug = slugifySaintId(parsed.name, SOURCE_URL);
assert.ok(slug.includes('frassinetti') || slug.includes('paula'));

const empty = parseSaintBioHtml(
...
// --- feast day helper ---
assert.strictEqual(parseFeastDayEs('18 de julio: San Bruno'), '07-18');
assert.strictEqual(parseFeastDayEs('/es/santos/07/18.saints.js'), '07-18');
assert.strictEqual(parseFeastDayEs('1 de enero: María'), '01-01');

// --- Vatican News .saints.js ---
const vnText = fs.readFileSync(FIXTURE_VN, 'utf8');
const vn = parseVaticanNewsSaintsJs(
  vnText,
  '/es/santos/07/18.saints.js',
  'https://www.vaticannews.va',
);
assert.ok(vn.length >= 2, `expected ≥2 vn saints, got ${vn.length}`);
const brunoVn = vn.find(
  (s) => /Bruno/i.test(s.name) || /Bruno/i.test(s.displayName || ''),
);
...
const vnEmpty = parseVaticanNewsSaintsJs(
...
const vn404 = parseVaticanNewsSaintsJs(
  '<html><title>404 Resource</title></html>',
  '/es/santos/01/01.saints.js',
);
...
// --- vaticanstate RSS ---
const rssRows = parseVaticanStateRss(fs.readFileSync(FIXTURE_RSS, 'utf8'));
...
const rssEmpty = parseVaticanStateRss('<rss><channel></channel></rss>');
assert.strictEqual(rssEmpty.length, 0);

// --- vaticanstate item HTML ---
const item = parseVaticanStateItemHtml(
...
const item404 = parseVaticanStateItemHtml(
