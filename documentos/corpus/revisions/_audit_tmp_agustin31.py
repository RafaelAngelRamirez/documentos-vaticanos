#!/usr/bin/env python3
"""Sample + classify residual OCR defects for agustin-31-antimaniqueos-2-es."""
...
DOC_ID = "agustin-31-antimaniqueos-2-es"
...
PATTERNS = {
    ...
    "bac_chrome_frontmatter": re.compile(
        ...
        r"TOMO XXXI\b|AUTORES CRISTIANOS Declarada|MCMLXXX|MCML|"
        r"ESCRITOS ANTIMANIQU|CONTRA FAUSTO|ORDEN SISTEM)",
        re.I,
    ),
    ...
    "latin_mixed_into_spanish_body": re.compile(
        ...
        r"dicitur|dicit|huius|eius|eorum|ecclesiae|baptismo|faustus|manichae)\b",
        re.I,
    ),
    ...
    "page_headers_footers": re.compile(
        r"(?:—\s*\d+\s*—|\bP[aá]g(?:ina)?\.?\s*\d+|\bVOL(?:UMEN)?\.?\s*XXXI\b|"
        r"SAN AGUST[IÍ]N\s*[-–—]\s*\d+|OBRAS DE SAN AGUST|ESCRITOS ANTIMANIQU|"
        r"CONTRA FAUSTO\s*[-–—]?\s*\d+)",
        re.I,
    ),
}
...
def refine_snip(text: str, c: str) -> str:
    ...
    if c == "glued_words":
        ...
        if m:
            return snippet(text, m)
        m = re.search(rf"[{LOWER}]{{3,}}[{UPPER}][{LOWER}]{{2,}}", text)
        if m:
            return snippet(text, m)
    if c == "accent_corruption":
        ...
        if m:
            return snippet(text, m)
    if c == "index_noise":
        return snippet(text, width=140)
    if c == "other_illegible":
        return snippet(text, width=140)
    return snippet(text)
...
def main() -> int:
    ...
    findings = []
    findings.append(
        f"Pack BAC XXXI antimaniqueos-2 ES (Contra Fausto): {total} units; ocr-punct-v2 changed 257, ocr-abc-v2 changed 261 (spaced collapse)."
    )
    findings.append(
        f"Front noise units 0–119 with ≥1 defect class: {front_noise}; residual after ocr-abc residualBodyNoiseUnits=14, junkRunHits=3, tocLeaderHits=1."
    )
    ...
    if full_counts.get("internal_word_period"):
        findings.append(
            f"internal_word_period left unrepaired by design ({full_counts['internal_word_period']} units; ocr-punct after count=18)."
        )
    if full_counts.get("index_noise"):
        findings.append(
            f"Tail index zone has index_noise in {full_counts['index_noise']} units."
        )
    ...
    if full_counts.get("glued_words"):
        findings.append(
            f"glued_words residual in {full_counts['glued_words']} units (ocr-abc only partial particle splits)."
        )
    findings.append(
        f"Full-scan high-severity units (≥35): {high}/{total}; body_avg_severity≈{body_avg:.1f}; avg_severity≈{avg_sev:.1f}."
    )
    if full_counts.get("latin_mixed_into_spanish_body"):
        findings.append(
            f"latin_mixed_into_spanish_body in {full_counts['latin_mixed_into_spanish_body']} units — treat carefully (citations vs dual-stream bleed)."
        )
    ...
    if full_counts.get("digit_glued_tokens"):
        suggested.append(
            "Split digit-letter glue conservatively: /(\\d)([A-Za-záéíóúñ]{3,})/ → '$1 $2' and /([A-Za-záéíóúñ]{3,})(\\d{2,})/ → '$1 $2' excluding biblical book codes (Gn, Ex, Mt…)."
        )
    if full_counts.get("page_headers_footers"):
        suggested.append(
            "Blank or strip short page-header units matching /^(—\\s*)?\\d{1,4}(\\s*—)?$/ or repeating 'OBRAS DE SAN AGUSTÍN' / 'CONTRA FAUSTO' running heads (keep unitIndex slot)."
        )
    ...
    if full_counts.get("internal_word_period"):
        suggested.append(
            "Do NOT auto-join lowercase.period.lowercase globally; optionally whitelist mid-word OCR splits only with dictionary check — leave legitimate abbreviations alone."
        )
    suggested.append(
    ...
    print(json.dumps({k: report[k] for k in report if k != "_debug"}, ensure_ascii=False, indent=2))
    print("---DEBUG---", file=sys.stderr)
    print(json.dumps(report["_debug"], ensure_ascii=False, indent=2), file=sys.stderr)
    return 0
