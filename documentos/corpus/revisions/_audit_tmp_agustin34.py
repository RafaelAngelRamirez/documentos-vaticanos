#!/usr/bin/env python3
"""Sample + classify residual OCR defects for agustin-34-antidonatistas-3-es."""
...
DOC_ID = "agustin-34-antidonatistas-3-es"
...
PATTERNS = {
    "toc_dotted_garbage": re.compile(r"(?:\.{4,}|\. \. \. \.|\t\.+\t|…{2,}|_{4,}|[oncrimONCRIM]{10,})"),
    "bac_chrome_frontmatter": re.compile(
        ...
        r"TOMO XXXIV|AUTORES CRISTIANOS Declarada|MCMXCIV|MCML|EDITORIAL CAT[OÓ]LICA|"
        r"Impreso en España|ISBN)",
        re.I,
    ),
    ...
    "digit_glued_tokens": re.compile(
        rf"(?:(?<![0-9./])\d{{1,3}}[{LETTER}]{{3,}}|[{LETTER}]{{3,}}\d{{2,}}(?![0-9./]))"
    ),
    ...
    "latin_mixed_into_spanish_body": re.compile(
        ...
        r"dicitur|dicit|huius|eius|eorum|ecclesiae|baptismo|baptismus|"
        r"episcopus|episcopi|unde|ergo|tamen|nihil|omnium)\b",
        re.I,
    ),
    ...
    "page_headers_footers": re.compile(
        r"(?:—\s*\d+\s*—|\bP[aá]g(?:ina)?\.?\s*\d+|\bVOL(?:UMEN)?\.?\s*XXXIV\b|"
...
def classify_unit(text: str, unit_index: int, total: int) -> list[str]:
    if not text:
        return []
    if text.strip() == PLACEHOLDER or text.strip().startswith("[OCR:"):
    ...
    if re.search(r"\b(?:LOSSEÑORES\w*|DELA[A-ZÁÉÍÓÚÑ]{2,}|PARAEL\w+|[A-ZÁÉÍÓÚÑ]{14,})\b", text):
        glued = True
    if re.search(rf"[{LOWER}]{{4,}}[{UPPER}][{LOWER}]{{3,}}[{UPPER}]", text):
        glued = True
    elif re.search(rf"[{LOWER}]{{5,}}[{UPPER}][{LOWER}]{{4,}}", text):
        glued = True
    if re.search(
    ...
    corrupt = re.findall(
        ...
        r"BIBLICGRAFIA|VIIT|XXVIIL|TT\.\s*TI|qiie|qne|baptisrno|espiritn|"
        r"despnes|tarnbien|Igiesia|dONAtiStaS|IgleSiA)\b",
        text,
    )
    if len(missing) >= 2 or corrupt:
        classes.append("accent_corruption")

    if PATTERNS["page_headers_footers"].search(text) and len(text) < 280:
    ...
    if is_tail:
        ...
        elif re.search(r"(?i)véase|v\.\s*t|cf\.|cfr\.|índice|indice|materias", text):
            classes.append("index_noise")
        elif len(text) < 100 and re.search(r"\d", text) and nums >= 2:
            classes.append("index_noise")
        elif re.search(r"[oncrim]{8,}", text, re.I):
            classes.append("index_noise")
...
def refine_snip(text: str, c: str) -> str:
    pat_map = {
        ...
        "latin_mixed_into_spanish_body": PATTERNS["latin_mixed_into_spanish_body"],
    }
    pat = pat_map.get(c)
    if pat:
        m = pat.search(text)
    ...
    if c == "glued_words":
        m = re.search(
            ...
            text,
        ) or re.search(rf"[{LOWER}]{{4,}}[{UPPER}][{LOWER}]{{3,}}", text)
        if m:
            return snippet(text, m)
    if c == "accent_corruption":
        m = re.search(
            r"\b(?:Escrriros|Escurros|Esckrrros|BIBLICGRAFIA|ademas|despues|tambien|espiritu|qiie|qne|Igiesia|dONAtiStaS)\b",
            text,
            re.I,
        )
        if m:
            return snippet(text, m)
    if c == "index_noise":
        m = re.search(r"(?i)índice|indice|véase|materias|\d", text)
        if m:
            return snippet(text, m)
    return snippet(text)
...
def main() -> int:
    ...
    findings = []
    findings.append(
        f"Pack BAC XXXIV antidonatistas-3 ES: {total} units; ocr-punct-v2 changed 563, ocr-abc-v2 changed 245 (spaced collapse)."
    )
    findings.append(
        f"Front matter: BAC chrome + multiple [OCR: índice…] placeholders (unitsExcludedGarbage=0 but placeholders present in content)."
    )
    ...
    if full_counts.get("internal_word_period"):
        findings.append(
            f"internal_word_period left unrepaired by design ({full_counts['internal_word_period']} units; ocr-punct after=13)."
        )
    if full_counts.get("index_noise"):
        findings.append(
            f"Tail index zone: index_noise in {full_counts['index_noise']} units (ocr-abc residualBodyNoiseUnits=8, tocLeaderHits=3)."
        )
    if full_counts.get("toc_dotted_garbage"):
        findings.append(
            f"toc_dotted_garbage / OCR placeholders: {full_counts['toc_dotted_garbage']} units (leaders or [OCR: índice…] slots)."
        )
    if full_counts.get("spaced_or_shredded_letters", 0) == 0:
        findings.append(
            "spaced-letter runs appear fully collapsed by ocr-abc-v2 (0 residual spaced runs in full scan)."
        )
    ...
    if full_counts.get("latin_mixed_into_spanish_body"):
        findings.append(
            f"latin_mixed_into_spanish_body in {full_counts['latin_mixed_into_spanish_body']} units (facing-page CSEL/Latin bleed into ES body)."
        )
    findings.append(
        f"Full-scan high-severity units (≥35): {high}/{total}; body_avg_severity≈{body_avg:.1f}; front_noise_units={front_noise}."
    )
    ...
    if full_counts.get("page_headers_footers"):
        suggested.append(
            "Blank or strip short page-header units matching /^(—\\s*)?\\d{1,4}(\\s*—)?$/ or repeating 'OBRAS DE SAN AGUSTÍN' / 'ESCRITOS ANTIDONATISTAS' running heads (keep unitIndex slot)."
        )
    if full_counts.get("bac_chrome_frontmatter") or full_counts.get("accent_corruption"):
        suggested.append(
            "Front-matter only: map high-confidence shredded title tokens (Escrriros→Escritos, Escurros→Escritos, BIBLICGRAFIA→BIBLIOGRAFIA) via fixed dict; do not touch body theology."
        )
    ...
    if full_counts.get("toc_dotted_garbage") or full_counts.get("index_noise"):
        suggested.append(
            "Mark remaining TOC/index dotted-leader units with OCR_GARBAGE_PLACEHOLDER (stable index) when leader density high and unit short; leave existing [OCR: índice…] slots."
        )
    if full_counts.get("internal_word_period"):
        suggested.append(
    ...
    suggested.append(
        "No unit reordering; never paraphrase theological body. Prefer re-OCR residualBodyNoise pages (dual-stream LA∥ES) over creative repair."
    )
    ...
    report = {
        ...
        "_debug": {
            ...
            "body_noise_units_sev25": body_noise,
